  ...
  dp: "bg-purple-500 text-white",
	...
	const filteredproblems=problems.filter(problem=>{
		...
		const tagMatch=filter.tag==='all'||problem.tags===filter.tag;
		 statusMatch=filter.status==='all'||solvedproblem.some(sp=>sp._id===problem._id);
		return difficultyMatch&&tagMatch&&statusMatch;
	})
	return (
		<div className='min-h-screen bg-base-200'>
			{/* hiiiii  supriya */}
     <Navbar filter={filter} setfilter={setfilter} isUser={user?.role === 'user'}></Navbar>
      {/* main container */}
				...
				<div className='grid grid-cols-1 gap-4 '>

					{
						...
						filteredproblems.map((problem)=>{
							...
							const issolved = Array.isArray(solvedproblem) && solvedproblem.some(sp => sp._id === problem._id);
...
<div className="flex gap-4 pl-8 ml-4  items-center justify-center ">
    {problem.tags.map(tag => (
      <span 
        key={tag} 
        className={`   px-2 py-1 rounded-full text-xs font-semibold font-lg  flex  items-center justify-center ${tagColors[tag.toLowerCase()] || 'bg-gray-500 text-white'}`}>
        {tag}
      </span>
    ))}
  </div>
									...
									<p className='text-sm mb-4'>
										{/* Status:{""} */}
										{
											issolved?(
												...
												<span className='text-green-600 font-semibold'>Unsolved </span>
